...
- (void)handleInterruption:(NSNotification *)notification
{
    ...
    DDLogVerbose(@"Session interrupted > --- %s ---\n", theInterruptionType == AVAudioSessionInterruptionTypeBegan ? "Begin Interruption" : "End Interruption");
    ...
    if (theInterruptionType == AVAudioSessionInterruptionTypeEnded) {
        ...
        if (nil != error) DDLogVerbose(@"AVAudioSession set active failed with error: %@", error);
    }
}
...
- (void)handleRouteChange:(NSNotification *)notification
{
    ...
    DDLogVerbose(@"Route change:");
    switch (reasonValue) {
    case AVAudioSessionRouteChangeReasonNewDeviceAvailable:
        DDLogVerbose(@"     NewDeviceAvailable");
        break;
    case AVAudioSessionRouteChangeReasonOldDeviceUnavailable:
        DDLogVerbose(@"     OldDeviceUnavailable");
        break;
    case AVAudioSessionRouteChangeReasonCategoryChange:
        DDLogVerbose(@"     CategoryChange");
        DDLogVerbose(@" New Category: %@", [[AVAudioSession sharedInstance] category]);
        break;
    case AVAudioSessionRouteChangeReasonOverride:
        DDLogVerbose(@"     Override");
        break;
    case AVAudioSessionRouteChangeReasonWakeFromSleep:
        DDLogVerbose(@"     WakeFromSleep");
        break;
    case AVAudioSessionRouteChangeReasonNoSuitableRouteForCategory:
        DDLogVerbose(@"     NoSuitableRouteForCategory");
        break;
    default:
        DDLogVerbose(@"     ReasonUnknown");
    }
    
    DDLogVerbose(@"Previous route:\n");
    DDLogVerbose(@"%@", routeDescription);
}
...
- (void)applicationDidFinishLaunching:(UIApplication *)application {
    ...
    try {
        ...
        DDLogInfo(@"output channel count: %@", @(sessionInstance.maximumOutputNumberOfChannels));
